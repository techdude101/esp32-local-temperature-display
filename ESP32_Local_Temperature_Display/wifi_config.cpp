...
bool get_wifi_credentials_from_eeprom(char* ssid, char* password) {
  ...
  const bool configured = (bool)EEPROM.read(CONFIG_FLAG_OFFSET);
  EEPROM.end();
...
void get_wifi_psk_from_eeprom(char* password) {
  ...
  for (int i = 0; i < 63; i++) {
    password[i] = (char)EEPROM.read((i + PSK_OFFSET));
  }
  return;
}
...
int save_url_to_eeprom(char* url) {
  ...
  for (int i = 0; i < 255; i++) {
    EEPROM.write((i + WIFI_CONFIG_OFFSET), url[i]);
  }
...
void get_url_from_eeprom(char* url) {
  EEPROM.begin(EEPROM_SIZE);
  for (int i = 0; i < 255; i++) {
    url[i] = (char)EEPROM.read(i + WIFI_CONFIG_OFFSET);
  }
  EEPROM.end();
}